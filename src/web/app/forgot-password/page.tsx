...
const particles = [
  ...
  { left: "40%", dur: "14s", delay: "-4s", size: 4, op: 0.6 },
  { left: "65%", dur: "10s", delay: "-1s", size: 2, op: 0.3 },
  { left: "80%", dur: "11s", delay: "-6s", size: 3, op: 0.5 },
];
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#07070f", fontFamily: "'Syne',sans-serif", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <section style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "30px 16px", position: "relative", zIndex: 5 }}>
        <div style={{ width: "100%", maxWidth: 380, background: "rgba(14,10,28,0.88)", border: "1px solid rgba(100,60,255,0.2)", borderRadius: 22, padding: "38px 34px 32px", backdropFilter: "blur(20px)", boxShadow: "0 30px 80px rgba(80,40,200,0.18)", animation: "slideUp .7s cubic-bezier(.16,1,.3,1) both" }}>
          ...
          <div style={{ width: 68, height: 68, borderRadius: "50%", margin: "0 auto 18px", background: "linear-gradient(145deg,#7040ff,#4020b0)", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid rgba(140,90,255,0.4)" }}>
            <Mail size={28} color="rgba(255,255,255,0.9)" strokeWidth={1.8} />
          </div>

          <h2 style={{ textAlign: "center", color: "#ddd0ff", fontSize: 19, fontWeight: 800, letterSpacing: 0.5, marginBottom: 5 }}>
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              {error && (
                <p style={{ color: "rgba(240,100,100,0.85)", fontFamily: "'Space Mono',monospace", fontSize: 11, marginTop: 10 }}>
                  {error}
                </p>
              )}
              ...
              <button
                ...
                disabled={loading}
                className="dh-btn"
                style={{ width: "100%", height: 46, marginTop: 22, background: loading ? "rgba(100,60,255,0.3)" : "linear-gradient(135deg,#7040ff,#5020e0)", border: "none", borderRadius: 11, color: "white", fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 800, letterSpacing: 4, textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", boxShadow: "0 4px 24px rgba(90,40,220,0.4)", transition: "transform .15s, box-shadow .2s" }}
              >
                {loading ? "Enviando..." : "Enviar código"}
              </button>

              <p style={{ textAlign: "center", fontFamily: "'Space Mono',monospace", fontSize: 11, color: "rgb(185,163,254)", marginTop: 20 }}>
                <span onClick={() => router.push("/login")} style={{ color: "#dfd6f5", cursor: "pointer", textDecoration: "underline" }}>
                  Volver al login
                </span>
              </p>
            </form>
