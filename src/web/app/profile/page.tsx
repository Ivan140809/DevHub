  ...
  return (
    <main style={{ minHeight:"100vh", background:"#07070f", fontFamily:"'Syne',sans-serif", position:"relative", overflow:"hidden", display:"flex", flexDirection:"column" }}>
      ...
      <section style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"36px 20px", position:"relative", zIndex:5 }}>
        <div style={{ width:"100%", maxWidth:780, background:"rgba(14,10,28,.9)", border:"1px solid rgba(100,60,255,.22)", borderRadius:22, padding:"42px 42px 36px", backdropFilter:"blur(20px)", boxShadow:"0 0 0 1px rgba(255,255,255,.03) inset,0 30px 80px rgba(80,40,200,.2)", animation: mounted ? "slideUp .7s cubic-bezier(.16,1,.3,1) both" : "none" }}>

          {/* Avatar */}
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16, marginBottom:36 }}>
            <div style={{ position:"relative", width:130, height:130, display:"flex", alignItems:"center", justifyContent:"center" }}>
              {mounted && <div style={{ position:"absolute", inset:-8, borderRadius:"50%", border:"1px solid rgba(120,70,255,.25)", animation:"ringPulse 3s ease-in-out infinite" }} />}
              <div style={{ width:130, height:130, borderRadius:"50%", background:"linear-gradient(145deg,#7040ff,#4020b0)", display:"flex", alignItems:"center", justifyContent:"center", border:"2px solid rgba(140,90,255,.5)", animation: mounted ? "avatarGlow 3s ease-in-out infinite" : "none", position:"relative", zIndex:1 }}>
                <User size={58} color="rgba(255,255,255,.9)" strokeWidth={1.5} />
              </div>
            </div>
            <div style={{ color:"#ddd0ff", fontSize:18, fontWeight:800, letterSpacing:.5 }}>Mi Perfil</div>
            <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, letterSpacing:3, textTransform:"uppercase", color:"rgba(160,130,255,.45)", background:"rgba(100,60,255,.1)", border:"1px solid rgba(100,60,255,.2)", padding:"4px 14px", borderRadius:999 }}>
              DevHub Member
            </div>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
            ...
            <Field label="Preferencias"       placeholder="Frontend, Backend..." />
            <Field label="Correo electrónico" placeholder="tu@email.com"        type="email" />
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", gap:12, marginTop:26 }}>
            <button style={{ height:40, padding:"0 24px", background:"transparent", border:"1px solid rgba(100,60,255,.3)", borderRadius:10, color:"rgba(180,150,255,.6)", fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700, letterSpacing:2, textTransform:"uppercase", cursor:"pointer" }}>
              Cancelar
            </button>
            <button style={{ height:40, padding:"0 28px", background:"linear-gradient(135deg,#7040ff,#5020e0)", border:"none", borderRadius:10, color:"white", fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:800, letterSpacing:3, textTransform:"uppercase", cursor:"pointer", boxShadow:"0 4px 20px rgba(90,40,220,.4)" }}>
              Guardar cambios
            </button>
          </div>
        </div>
...
function Field({ label, placeholder, type="text" }: { label:string; placeholder:string; type?:string }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
      ...
      <input type={type} placeholder={placeholder} style={{ height:44, background:"rgba(255,255,255,.04)", border:"1px solid rgba(100,60,255,.18)", borderRadius:10, padding:"0 14px", fontFamily:"'Space Mono',monospace", fontSize:13, color:"#ddd0ff", outline:"none" }} />
    </div>
  );
}
